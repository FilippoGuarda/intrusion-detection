...
def change_detection(video_path, bg, threshold,frame):
    ...
    while (cap.isOpened()):
        ...
        edges[np.logical_not(thresh)] = np.asarray(0)
        cv2.imshow('e', edges)
        ...
        cv2.imshow('a0', im_floodfill_inv)
        out = opening | im_floodfill_inv
        cv2.imshow('floodfill', out)
        closing = cv2.morphologyEx(out, cv2.MORPH_CLOSE,  cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(7,7)), iterations = 2)
