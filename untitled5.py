...
# -*- coding: utf-8 -*-
"""
Created on Wed May  5 21:27:44 2021
...
# -*- coding: utf-8 -*-
"""
Created on Wed May  5 20:05:22 2021
...
def change_detection(video_path, bg, threshold,frame):
    ...
    while (cap.isOpened()):
        ...
        closing = cv2.morphologyEx(im_out, cv2.MORPH_CLOSE,  cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(5,5)), iterations = 3)
        cv2.imshow('closing', closing)
        opening = cv2.morphologyEx(closing, cv2.MORPH_OPEN,  cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(1,1)), iterations = 1)
        ...
        #edges = frame.astype(np.uint8)
        #edges[np.logical_not(opening)] = np.asarray([-255])
        #cv2.imshow('e', edges)
        #find edges and use as a mask for floodfill
        ...
        _, contours, hierarchy = cv2.findContours(opening, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_KCOS)
        final = closing
